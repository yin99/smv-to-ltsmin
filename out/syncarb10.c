...
int smv_get_actions (void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg) {
	...
	case 0: {
		...
		e1_grant_out = !in->e1_Request && grant_out;
		...
		e1_ack_out = in->e1_Request && (in->e1_Persistent && in->e1_Token || grant_out);		
		...
		e10_override_out = override_out || (in->e10_Persistent && in->e10_Token);
