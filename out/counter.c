...
int smv_get_actions (void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg) {
	...
	case 0: {
		...
		int bit1_carry_out;
		int bit2_carry_out;
		bit0_carry_out = in->bit0_value && TRUE;
		bit1_carry_out = in->bit1_value && bit0_carry_out;
		bit2_carry_out = in->bit2_value && bit1_carry_out;
		out->bit0_value = FALSE;
		cpy[((int *)&out->bit0_value - (int *)out)] = 0;
		out->bit1_value = FALSE;
		cpy[((int *)&out->bit1_value - (int *)out)] = 0;
		out->bit2_value = FALSE;
		cpy[((int *)&out->bit2_value - (int *)out)] = 0;
	...
	case 1: {
		...
		bit1_carry_out = in->bit1_value && bit0_carry_out;
		bit2_carry_out = in->bit2_value && bit1_carry_out;
		out->bit0_value = in->bit0_value ^ TRUE;
		cpy[((int *)&out->bit0_value - (int *)out)] = 0;
		out->bit1_value = in->bit1_value ^ bit0_carry_out;
		cpy[((int *)&out->bit1_value - (int *)out)] = 0;
		out->bit2_value = in->bit2_value ^ bit1_carry_out;
		cpy[((int *)&out->bit2_value - (int *)out)] = 0;
...
void pins_model_init(model_t m) {
	...
	lts_type_set_state_name(ltstype, 0,"bit0_value");
	lts_type_set_state_typeno(ltstype, 0,int_type);
	lts_type_set_state_name(ltstype, 1,"bit1_value");
	lts_type_set_state_typeno(ltstype, 1,int_type);
	lts_type_set_state_name(ltstype, 2,"bit2_value");
