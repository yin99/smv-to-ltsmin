...
int smv_get_actions (void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg) {
	...
	case 0: {
		...
		out->state = ready;
		cpy[((int *)&out->state - (int *)out)] = 0;
		out->virtual_init = FALSE;
		cpy[((int *)&out->virtual_init - (int *)out)] = 0;
		out->request = smv_non_deterministic("request",2,FALSE,TRUE);
		cpy[((int *)&out->request - (int *)out)] = 0;
	...
	case 1: {
		memcpy(out, in, sizeof(state_t));
		out->state =  ( in->state == ready && in->request ? busy : smv_non_deterministic("state",2,ready,busy) ) ;
		cpy[((int *)&out->state - (int *)out)] = 0;
		out->request = smv_non_deterministic("request",2,FALSE,TRUE);
		cpy[((int *)&out->request - (int *)out)] = 0;
...
void pins_model_init(model_t m) {
	...
	lts_type_set_state_name(ltstype, 0,"request");
	lts_type_set_state_typeno(ltstype, 0,int_type);
	lts_type_set_state_name(ltstype, 1,"state");
	lts_type_set_state_typeno(ltstype, 1,int_type);
	lts_type_set_state_name(ltstype, 2,"virtual_init");
