...
typedef struct state_t {
	boolean gate1_output;
	boolean gate1_running;
	boolean gate2_output;
	boolean gate2_running;
	boolean gate3_output;
	boolean gate3_running;
...
int smv_get_actions (void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg) {
	...
	int cpy[7] = { 1,1,1,1,1,1,1,};
	switch(t) {
	case 0: {
		memcpy(out, in, sizeof(state_t));
		out->gate1_output = FALSE;
		cpy[((int *)&out->gate1_output - (int *)out)] = 0;
		out->gate2_output = FALSE;
		cpy[((int *)&out->gate2_output - (int *)out)] = 0;
		out->gate3_output = FALSE;
		cpy[((int *)&out->gate3_output - (int *)out)] = 0;
	...
	case 1: {
		memcpy(out, in, sizeof(state_t));
		out->gate1_running = FALSE;
		cpy[((int *)&out->gate1_running - (int *)out)] = 0;
		out->gate1_output = !in->gate3_output;
		cpy[((int *)&out->gate1_output - (int *)out)] = 0;
	...
	case 2: {
		memcpy(out, in, sizeof(state_t));
		out->gate2_running = FALSE;
		cpy[((int *)&out->gate2_running - (int *)out)] = 0;
		out->gate2_output = !in->gate1_output;
		cpy[((int *)&out->gate2_output - (int *)out)] = 0;
	...
	case 3: {
		memcpy(out, in, sizeof(state_t));
		out->gate3_running = FALSE;
		cpy[((int *)&out->gate3_running - (int *)out)] = 0;
		out->gate3_output = !in->gate2_output;
		cpy[((int *)&out->gate3_output - (int *)out)] = 0;
	...
	case 4: {
		...
		out->gate1_running= TRUE;
		cpy[((int *)&out->gate1_running - (int *)out)] = 0;
		callback(arg,&transition_info,out,cpy);
		out->gate1_running= FALSE;
		cpy[((int *)&out->gate1_running - (int *)out)] = 1;
		out->gate2_running= TRUE;
		cpy[((int *)&out->gate2_running - (int *)out)] = 0;
		callback(arg,&transition_info,out,cpy);
		out->gate2_running= FALSE;
		cpy[((int *)&out->gate2_running - (int *)out)] = 1;
		out->gate3_running= TRUE;
		cpy[((int *)&out->gate3_running - (int *)out)] = 0;
		callback(arg,&transition_info,out,cpy);
		return 3;
	}
...
int smv_next_state(void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg){
	...
	case 1: {
		if(in->gate1_running == TRUE){
			smv_get_actions(model,1,in,callback,arg);
		}
	}
	case 2: {
		if(in->gate2_running == TRUE){
			smv_get_actions(model,2,in,callback,arg);
		}
	}
	case 3: {
		if(in->gate3_running == TRUE){
			smv_get_actions(model,3,in,callback,arg);
		}
	}
	case 4: {
		if(!((in->virtual_init == TRUE)||(in->gate1_running == TRUE)||(in->gate2_running == TRUE)||(in->gate3_running == TRUE))){
...
int smv_get_label(void* model, int g, state_t* in){
	switch (g){
		...
		case 1: return (in->gate1_running == TRUE) !=0;
		case 2: return (in->gate2_running == TRUE) !=0;
		case 3: return (in->gate3_running == TRUE) !=0;
		case 4: return !((in->virtual_init == TRUE)||(in->gate1_running == TRUE)||(in->gate2_running == TRUE)||(in->gate3_running == TRUE));
	}
}
...
void pins_model_init(model_t m) {
	...
	lts_type_set_state_name(ltstype, 0,"gate1_output");
	lts_type_set_state_typeno(ltstype, 0,int_type);
	lts_type_set_state_name(ltstype, 1,"gate1_running");
	lts_type_set_state_typeno(ltstype, 1,int_type);
	lts_type_set_state_name(ltstype, 2,"gate2_output");
	lts_type_set_state_typeno(ltstype, 2,int_type);
	lts_type_set_state_name(ltstype, 3,"gate2_running");
	lts_type_set_state_typeno(ltstype, 3,int_type);
	lts_type_set_state_name(ltstype, 4,"gate3_output");
	lts_type_set_state_typeno(ltstype, 4,int_type);
	lts_type_set_state_name(ltstype, 5,"gate3_running");
	lts_type_set_state_typeno(ltstype, 5,int_type);
	lts_type_set_state_name(ltstype, 6,"virtual_init");
	...
	lts_type_set_state_label_name (ltstype, 1, "now_gate1_running");
	lts_type_set_state_label_typeno (ltstype, 2, bool_type);
	lts_type_set_state_label_name (ltstype, 2, "now_gate2_running");
	lts_type_set_state_label_typeno (ltstype, 3, bool_type);
	lts_type_set_state_label_name (ltstype, 3, "now_gate3_running");
	...
	pins_chunk_put(m, action_type, chunk_str("gate1_running"));
	pins_chunk_put(m, action_type, chunk_str("gate2_running"));
	pins_chunk_put(m, action_type, chunk_str("gate3_running"));
