...
typedef enum smv_enum{
	 n1, t1, c1, n2, t2, c2, EMPTYENUM1, EMPTYENUM2,
}smv_enum;
...
int smv_get_actions (void* model, int t, state_t *in, void (*callback)(void* arg, transition_info_t *transition_info, state_t *out, int *cpy), void *arg) {
	...
	case 0: {
		...
		out->state1 = n1;
		cpy[((int *)&out->state1 - (int *)out)] = 0;
		out->state2 = n2;
		cpy[((int *)&out->state2 - (int *)out)] = 0;
		out->turn = 1;
		cpy[((int *)&out->turn - (int *)out)] = 0;
	...
	case 1: {
		memcpy(out, in, sizeof(state_t));
		out->state1 =  ( (in->state1 == n1) && (in->state2 == t2) ? t1 :  ( (in->state1 == n1) && (in->state2 == n2) ? t1 :  ( (in->state1 == n1) && (in->state2 == c2) ? t1 :  ( (in->state1 == t1) && (in->state2 == n2) ? c1 :  ( (in->state1 == t1) && (in->state2 == t2) && (in->turn == 1) ? c1 :  ( (in->state1 == c1) ? n1 : in->state1 )  )  )  )  )  ) ;
		cpy[((int *)&out->state1 - (int *)out)] = 0;
		out->state2 =  ( (in->state2 == n2) && (in->state1 == t1) ? t2 :  ( (in->state2 == n2) && (in->state1 == n1) ? t2 :  ( (in->state2 == n2) && (in->state1 == c1) ? t2 :  ( (in->state2 == t2) && (in->state1 == n1) ? c2 :  ( (in->state2 == t2) && (in->state1 == t1) && (in->turn == 2) ? c2 :  ( (in->state2 == c2) ? n2 : in->state2 )  )  )  )  )  ) ;
		cpy[((int *)&out->state2 - (int *)out)] = 0;
		out->turn =  ( (in->state1 == n1) && (in->state2 == t2) ? 2 :  ( (in->state2 == n2) && (in->state1 == t1) ? 1 : in->turn )  ) ;
		cpy[((int *)&out->turn - (int *)out)] = 0;
...
void pins_model_init(model_t m) {
	...
	lts_type_set_state_name(ltstype, 0,"state1");
	lts_type_set_state_typeno(ltstype, 0,int_type);
	lts_type_set_state_name(ltstype, 1,"state2");
	...
	lts_type_set_state_name(ltstype, 3,"virtual_init");
